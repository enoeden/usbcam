...
int fimc0_out_qbuf(int index) {
	...
	b.m.userptr=(int)cambuf;
	printf("idx : %d  cam buf start :%lx \n",index ,b.m.userptr);
...
void process_cam_to_fimc0_to_lcd() {
  ...
  printf("%d +\n", index);
	...
	memcpy(fb_buf, cam_buffers[index].start,768000);
	cam_cap_qbuf(index);
	printf("idx %d , fbbuf %lx ,cambuffers %lx  \n", index,fb_buf,cam_buffers[index].start);

	//printf("ready fimc0_out_qbuf %d\n", index);
