...
//  req.memory = V4L2_MEMORY_USERPTR;  
	...
	for (n_buffers = 0; n_buffers < ReqButNum; ++n_buffers) {
		cam_buffers[n_buffers].length = fimc0_out_buf_length;
		cam_buffers[n_buffers].start = malloc(fimc0_out_buf_length); //fimc0_out_buf[n_buffers].start;

		if (!cam_buffers[n_buffers].start) {
			fprintf(stderr, "Out of memory\n");
			exit(EXIT_FAILURE);
		}
	}
...
int fimc0_reqbufs() {
	...
	/* mmap DMABUF */
	////struct v4l2_plane plane;  //do not have this struct in 2.6 kernel
#if 0  
	CLEAR(plane);
	CLEAR(b);
	...
	for (n = 0; n < ReqButNum; ++n) {
		...
		//b.index = n;
		//printf("line:%d, cam_buffers[n].start:0x%08x\n",cam_buffers[n].start);
		//  void *addr = malloc(fimc0_out_buf_length + 64);
		//  addr = (void *)(((unsigned long)addr + 63)&(~63));
		fimc0_out_buf[n].start = fb_buf;//(void *)memalign(/* boundary */(size_t)page_size, (size_t)fimc0_out_buf_length);//malloc(fimc0_out_buf_length);
	...
	for (n = 0; n < ReqButNum; ++n) {
		...
		fimc0_out_buf[n].length = b.length;
	...
	fimc0_cap_buf_length = 800 * 480 * 2;
	printf("fimc0 capture:plane.length:%d\n", fimc0_cap_buf_length);
#endif  
	printf("%s -\n", __func__);
	return 0;
...
int fimc0_setfmt() {
	...
	//struct v4l2_pix_format *pix_mp = &stream_fmt.fmt.pix;
	//pix_mp->width = pix.width;
	//pix_mp->height = pix.height;
	//pix_mp->pixelformat = pix.pixelformat;
	//pix_mp->bytesperline = pix.bytesperline;
	////dump_format("fimc0-output", &stream_fmt);

	printf("%s -ready to set+\n", __func__);
	...
	struct v4l2_pix_format *pix_mp_f = &stream_fmt.fmt.pix;
	CLEAR(*pix_mp_f);
	////pix_mp_f->pixelformat = V4L2_PIX_FMT_RGB32;
	pix_mp_f->pixelformat = V4L2_PIX_FMT_RGB565;
	pix_mp_f->width = 800;
	pix_mp_f->height = 480;
	pix_mp_f->bytesperline = 0;
...
int start_capturing(int cam_fd) {
	...
	for (i = 0; i < n_buffer; i++) {
		...
		buf.index = i;
		buf.m.userptr = (unsigned long)cam_buffers[i].start;
		buf.length = cam_buffers[i].length;
	...
	for (i = 0; i < n_buffer; i++) {
		...
		if (b.memory == V4L2_MEMORY_USERPTR) {
			...
			b.m.userptr = (unsigned long) fimc0_cap_buf[i].start;
			b.length = (unsigned long) fimc0_cap_buf_length;
			b.bytesused = fimc0_cap_buf_length;
		}
		ret = ioctl(fimc0_fd, VIDIOC_QBUF, &b);
		if (ERR_ON(ret < 0, "fimc0: VIDIOC_QBUF: %s\n", ERRSTR))
			return -errno;
...
int cam_cap_qbuf(int index) {
	...
	buf.m.userptr = (unsigned long)cam_buffers[index].start;
	buf.length = cam_buffers[index].length;
	//  printf("cam qbuf:%d,userptr:0x%08x,length:%d\n",i, buf.m.userptr, buf.length);
	if(-1 == ioctl(cam_fd,VIDIOC_QBUF,&buf))
	{
		perror("cam Fail to ioctl 'VIDIOC_QBUF'");
...
int fimc0_out_qbuf(int index) {
	...
	{
		//  memcpy(fimc0_out_buf[index].start, cam_buffers[index].start, cam_buffers[index].length);
...
//  printf("%s +\n", __func__);  
	...
	////plane.length = (unsigned long)fimc0_cap_buf_length;
	////plane.bytesused = fimc0_cap_buf_length  ;
	b.m.userptr = (unsigned long) fimc0_cap_buf[index].start;
	b.length = (unsigned long) fimc0_cap_buf_length;
	b.bytesused = fimc0_cap_buf_length;
...
//  printf("%s +\n", __func__);  
	cam_cap_dbuf(&index);
	memcpy(fimc0_out_buf[index].start, cam_buffers[index].start,
			fimc0_out_buf_length);
...
static void *display_thread(void *pVoid) {
	...
	while (1) {
		sem_wait(&lcd_sem);
		count++;
		memcpy((void *) fb_buf, (void *) fimc0_cap_buf[fimc0_cap_index].start,
				num);
